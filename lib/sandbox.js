...
function sizeCheck(){
  ...
  for(var i = 1; i < 6; i++){
    if (document.getElementById(`sandbox-input-arg-${i}-1`).value > 40000){
      window.alert("You have at least one input array that has a length over 40,000. Bubble Sort runs in quadratic big-O time and therefore it will take too long to run. Please select an input array of less than 40,000 in length and click 'run' again.");
      okToRun = false;
    } else if (document.getElementById(`sandbox-input-arg-${i}-1`).value > 30000){
        window.alert("You have at least one input array that has a length over 30,000. This may cause a wait time of a few seconds while the algorithms to finish running. Please wait a few seconds for the results.");
      }
  }

  if(document.getElementById("sandbox-example").value === "subsets")
  for(var i = 1; i < 6; i++){
    if (document.getElementById(`sandbox-input-arg-${i}-1`).value > 21){
      window.alert("You have at least one input array that has a length over 21. Array Subsets runs in exponential big-O time and therefore it will take too long to run. Please select an input array of less than 22 in length and click 'run' again.");
      okToRun = false;
    }
  }

  if(document.getElementById("sandbox-example").value === "fibs")
  for(var i = 1; i < 6; i++){
    if (document.getElementById(`sandbox-input-arg-${i}-1`).value > 13000){
      window.alert("The resursive version of the Fibonacci algorithm reaches maximum stack size with an input over 13,000. Please select a smaller input and click 'run' again.");
      okToRun = false;
    }
  }

  if(document.getElementById("sandbox-example").value === "binarySearch")
  for(var i = 1; i < 6; i++){
    if (document.getElementById(`sandbox-input-arg-${i}-1`).value > 10000000){
      window.alert("You have at least one input array that has a length over 10,000,000. Binary Search is fast, but not that fast... please choose a smaller array to avoid wait times and click 'run' again.");
      okToRun = false;
    }
  }
  return okToRun;
...
function resizeInputs(){
  ...
  if(noneInputs[2] && noneInputs[1] && noneInputs[0]){
    for(var i = 0; i < 5; i++){
       inputBoxClass[i].style.height = "20px";
    }
  } else if(noneInputs[2] && noneInputs[1]){
    for(var i = 0; i < 5; i++){
       inputBoxClass[i].style.height = "55px";
    }
  } else if(noneInputs[2]){
    for(var i = 0; i < 5; i++){
